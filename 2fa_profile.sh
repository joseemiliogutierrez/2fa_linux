...
##
# Este script junto a su par, 2fa_bashrc.sh, son un conjunto de
...
while [ "${codigo_aleatorio}" != "${codigo_introducido}" ]; do
	codigo_introducido=$(zenity --entry --title="Segundo Factor de Autenticación" --text="\nIntroduzca el código que se ha enviado al correo electrónico configurado por el propietario de la cuenta.\n" --ok-label=Confirmar 2> /dev/null)
done
